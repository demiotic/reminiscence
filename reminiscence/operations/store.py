"""Storage and batch operations with multimodal support."""
...
from ..types import CacheEntry, MultiModalInput
...
class StorageOperations:
    """Handles cache storage and batch operations with multimodal support."""

    def __init__(
        self, storage: Any, embedder: Any, eviction: Any, config: Any, metrics: Any
    ) -> None:
    ...
    def store(
        ...
        Args:
            query: MultiModalInput to cache.
        ...
        logger.debug(
            "store_start",
            query_preview=query_text[:50] if query_text else "<no-text>",
        ...
        try:
            ...
            # Determine query mode (moved before storage to prepare entry)
            embedding = None
            actual_mode = query_mode
            if query_mode == "auto" and query_text:
            ...
            if actual_mode == "semantic" and query_text:
            ...
            # Store entry (add first, evict later - relaxed eviction for thread safety)
            ...
            entry_id = self._generate_entry_id(query_text, context)
            ...
            if self.config.max_entries:
                ...
                if current_count > eviction_threshold:
                    # Evict multiple entries to get back below max_entries
                    excess = current_count - self.config.max_entries
                    evicted_count = 0

                    logger.debug(
                        "relaxed_eviction_triggered",
                    ...
                    for _ in range(excess):
                        try:
                            ...
                            logger.warning(
                                "eviction_failed_during_cleanup",
                                error=str(e),
                                evicted_so_far=evicted_count,
                            )
                            break

                    if evicted_count > 0:
                        logger.info(
                            "relaxed_eviction_complete",
    ...
    def store_batch(
        ...
        query_texts = [q.text or "" for q in queries]
        ...
        valid_texts = [query_texts[i] for i in valid_entries]
        ...
        embed_start = time.time()
        embeddings: List[Optional[Any]] = [None] * len(valid_texts)
        ...
        if semantic_indices:
            semantic_queries = [valid_texts[i] for i in semantic_indices]
